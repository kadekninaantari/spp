...
if($_POST){
    if($_POST['aksi']=='tambah-berdasarkan-kelas'){
        ...
        // Mengecek Siswa Yang Sesui Tingkat & Jurusan
        $sql_siswa="SELECT * FROM siswa WHERE dihapus_pada IS NULL AND tingkat=$tingkat AND id_jurusan=$id_jurusan"; $query_siswa=mysqli_query($koneksi,$sql_siswa,); $jumlah_siswa=mysqli_num_rows($query_siswa); 
        //echo "Ditemukan ".$jumlah_siswa." Siswa Cocok";

        // Mengecek Biaya Yang Cocok
    $sql_biaya="SELECT * FROM biaya WHERE dihapus_pada IS NULL AND id_periode=$id_periode AND
    tingkat=$tingkat AND id_jurusan=$id_jurusan";
    ...
    //echo "<br>Ditemukan".$jumlah_biaya."Biaya Cocok";
        ...
        if($jumlah_siswa>=1 && $jumlah_biaya>=1){
            while($siswa=mysqli_fetch_array($query_siswa)){
                ...
                while($biaya=mysqli_fetch_array
                ($query_biaya)){
